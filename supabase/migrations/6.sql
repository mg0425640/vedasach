/*
# VedaWell Ads Complete System & Articles SEO Enhancement

## Overview
Comprehensive ads management system with category-specific slots, global ads,
auth/popup ads, promoted content, and full SEO fields on articles.

## New Tables

### 1. category_ads
Category-specific advertisement slots for article pages.
Slots: banner-1, banner-2, banner-3 (horizontal banners) and
side-image-1, side-image-2, side-image-3 (sidebar images).
Each ad is tied to one or more categories and subcategories.

### 2. global_ads
Site-wide ads shown on homepage and across the entire site.
Configurable to show on: home, all_pages, specific sections.

### 3. auth_popup_ads
Ads displayed inside authentication modals and popup windows.
Lightweight format suitable for modal context.

### 4. promoted_content
Sponsored/promoted content items shown at the end of articles.
Grid format (like Taboola/Outbrain) with image, sponsor name, title.
Category-targeted for relevance.

## Modified Tables

### articles
Added bilingual support, SEO meta fields, engagement counters,
Hindi content fields.

## Security
- All new tables: RLS enabled, public read (anon + authenticated)
- Admins update via service role key
*/

-- ─── DROP OLD ads TABLE IF EXISTS (replace with better structure) ──────────────
-- We keep the old ads table intact and add new structured tables
...
-- ─── CATEGORY_ADS TABLE ───────────────────────────────────────────────────────
-- Ads for category and article detail pages
-- Slots: banner-1, banner-2, banner-3 (top/mid/bottom banners)
--        side-image-1, side-image-2, side-image-3 (sidebar ads)
CREATE TABLE IF NOT EXISTS category_ads (
  ...
  link_url text,
  -- Slot determines WHERE on the page this ad appears
  slot text NOT NULL CHECK (slot IN ('banner-1','banner-2','banner-3','side-image-1','side-image-2','side-image-3')),
  -- Category targeting (which category pages to show this ad on)
  ...
  -- Also show on article detail pages of the same category
...
-- ─── GLOBAL_ADS TABLE ────────────────────────────────────────────────────────
-- Ads for homepage and site-wide placement
...
-- Ads for login/signup modals and popup windows
...
-- Sponsored/promoted content shown at bottom of articles (like Taboola/Outbrain)
-- Grid of 3–6 items per row
...
-- ─── SEED: CATEGORY ADS (18 ads across all slots and categories) ──────────────
...
('Home Remedies eBook', 'घरेलू नुस्खे ईबुक', '500+ home remedies in one book', 'https://images.pexels.com/photos/4226896/pexels-photo-4226896.jpeg', 'https://vedawell.in/shop/ebook', 'banner-1', true, 1);
...
-- ─── SEED: PROMOTED CONTENT (12 items) ───────────────────────────────────────
