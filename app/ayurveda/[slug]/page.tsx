...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'Ayurveda').eq('is_published', true);
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Ayurveda Guide – vedasach',
    description: a?.meta_description || a?.excerpt || 'Ayurvedic remedies and natural healing on vedasach.',
