...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'Lifestyle').eq('is_published', true);
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Lifestyle – vedasach',
    description: a?.meta_description || a?.excerpt || 'Lifestyle tips, trends, and inspiration for modern living.',
...
export default function LifestyleArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="lifestyle" categoryLabel="Lifestyle" />;
}
