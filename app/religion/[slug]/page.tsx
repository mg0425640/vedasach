...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'Religion').eq('is_published', true);
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Religion – vedasach',
    description: a?.meta_description || a?.excerpt || 'Religion news, spiritual guidance, and articles on faith traditions.',
...
export default function ReligionArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="religion" categoryLabel="Religion" />;
}
