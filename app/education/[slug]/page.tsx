...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'Education').eq('is_published', true);
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Education – vedasach',
    description: a?.meta_description || a?.excerpt || 'Education news, career guidance, exams, and learning resources.',
...
export default function EducationArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="education" categoryLabel="Education" />;
}
