...
export async function generateStaticParams() {
  const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  const { data } = await sb.from('articles').select('slug').eq('category', 'Entertainment').eq('is_published', true);
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: a?.meta_title || a?.title || 'Entertainment – vedasach',
    description: a?.meta_description || a?.excerpt || 'Entertainment news, celebrity updates, movie reviews, and more.',
...
export default function EntertainmentArticlePage({ params }: Props) {
  return <ArticleDetailLayout slug={params.slug} categorySlug="entertainment" categoryLabel="Entertainment" />;
}
